                ...
                <table class="table" align="center" style="font-size: 0.7rem;" style="width: 100%;">
                    <thead class="table table-light">
                        ...
                        <tr>
                            ...
                            ?>
                                <th colspan="7"><?= date_format($tglmulai, 'l'); ?></th>
                    ...
                    <tbody>
                        ...
                        <!-- cek $kelas apakah ada atau kosong -->
                        <?php if (!empty($kelas)) { ?>
                            <!-- jika ada -->
                            <!-- perulangan sesuai jumlah kelas -->
                            <?php for ($d = 0; $d < count($kelas); $d++) : ?>
                                ...
                                <tr>
                                    ...
                                    <!-- nama kelas -->
                                    <td><?= $kelas[$d]->nama_tingkatan . ' ' . $kelas[$d]->nama_jurusan . ' ' . $kelas[$d]->nomor_kelas; ?></td>

                                    <!-- jumlah siswa -->
                                    <!-- mengambil jumlah siswa sesuai dengan kelasnya -->
                                    <?php
                                    $id_kelas = $kelas[$d]->id;
                                    $siswa = $this->db->get_where('tbl_siswa', ['id_kelas' => $id_kelas]);
                                    // var_dump($siswa);
                                    $jmlSiswa = $siswa->num_rows();
                                    ?>
                                    <td><?= $jmlSiswa; ?></td>
                                    ...
                                    ?>
                                        ...
                                        <!--mengambil jumlah kehadiran, jumlah berdasarkan kelas-->
                                        <?php
                                       
                                        // mengambil kehadiran berdasarkan interval tanggal
                                        // $sql = "SELECT a.* FROM `record_kehadiran` AS `a` JOIN tbl_siswa AS b ON a.ids = b.id_siswa WHERE a.tanggal = '$today' AND status NOT IN ('bolos', 'alpa', 'izin', 'sakit')";

                                        // $res = $this->db->query($sql);

                                        // $jmlKehadiran = $res->num_rows();
                        ...
                        <?php } ?>
                        <!-- akhir if cek kelas ada atau tidak -->
                    </tbody>
                </table>
            </div>
        </div>
